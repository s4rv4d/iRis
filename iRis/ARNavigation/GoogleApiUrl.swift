...
class GoogleApiUrl {
  ...
  // MARK: - get url
    ...
    func getRouteUrl(source: LocationDetails, destination: LocationDetails, mode: String) -> String {
        return "\(baseUrl)/directions/json?origin=\(source.lat),\(source.lng)&destination=\(destination.lat),\(destination.lng)&mode=\(mode)&key=AIzaSyCH_MvUgsq0_pR_ehAKFChXbyEk5wK-dkA"
    }
}
